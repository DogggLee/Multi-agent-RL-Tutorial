...
def tanh_action_sample(dist):
    raw_action = dist.rsample()  # 用 rsample() 支持重参数
    action = torch.tanh(raw_action)
    # 修正 log_prob：加上 tanh 的导数项
...
class MAPPO_MPE:
    def __init__(self, args):
        ...
        # 添加一个变量保存所有智能体ID（在Runner中设置）
        self.all_agents = []  # 将在Runner中设置，保存所有智能体ID
    ...
    def choose_action(self, obs_dict, evaluate):
        with torch.no_grad():
            ...
            # 下方内容表示输出连续动作
            mean, std = self.actor(actor_inputs)
            dist = torch.distributions.Normal(mean, std)
            if evaluate:
                a_n = torch.tanh(mean)   # deterministic 动作也要 squash
                a_logprob_n = None
            else:
                a_n, a_logprob_n = tanh_action_sample(dist)
            # return a_n.numpy(), None if a_logprob_n is None else a_logprob_n.numpy()
            # 将动作张量转换为字典格式
            actions_dict = {}
            logprobs_dict = {}
            
            for i, agent_id in enumerate(self.env.agents):
                actions_dict[agent_id] = a_n[i].numpy()
                if a_logprob_n is not None:
                    logprobs_dict[agent_id] = a_logprob_n[i].item()  # 单个标量值
                
            if a_logprob_n is None:
    ...
    def train(self, replay_buffer, total_steps):
        ...
        for _ in range(self.K_epochs):
            for index in BatchSampler(SequentialSampler(range(self.batch_size)), self.mini_batch_size, False):
                ...
                # 下文代表连续动作
                mean_now, std_now = self.actor(actor_inputs[index])  # (mini_batch, ep_len, N, action_dim)
                dist_now = torch.distributions.Normal(mean_now, std_now)
                # 因为 replay buffer 里保存的 a_n 是经过 tanh 后的动作
                # 先对 a_n 做 atanh（反 tanh）拿到 raw_action
                eps = 1e-6
                a_n_raw = torch.atanh(torch.clamp(batch['a_n'][index], -1 + eps, 1 - eps))
                # log_prob 部分：
                log_prob = dist_now.log_prob(a_n_raw).sum(-1)
                log_prob -= (2 * (np.log(2) - a_n_raw - F.softplus(-2 * a_n_raw))).sum(-1)
                a_logprob_n_now = log_prob
                # entropy：用原始高斯分布的熵
